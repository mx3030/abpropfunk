    ...
    const methodAttributes = {
        ...
        'xaxis': (app,v) => app.setAxesVisible(v, app.getGraphicsOptions(1).axes.y.visible, app.getGraphicsOptions(1).axes.z.visible),
        'yaxis': (app,v) => app.setAxesVisible(app.getGraphicsOptions(1).axes.x.visible, v, app.getGraphicsOptions(1).axes.z.visible),
        'zaxis': (app,v) => app.setAxesVisible(app.getGraphicsOptions(1).axes.x.visible, app.getGraphicsOptions(1).axes.y.visible, v),
        ...
        'coordsystem': (app,v) => app.setCoordSystem(...splitlist(v).map(x=>parseFloat(x))),
        'axislabels': (app,v) => app.setAxisLabels(1, ...splitlist(v)),
        'xaxislabel': (app,v) => app.setAxisLabels(1, v, app.getGraphicsOptions(1).axes.y.label || '', app.getGraphicsOptions(1).axes.z.label || ''),
        'yaxislabel': (app,v) => app.setAxisLabels(1, app.getGraphicsOptions(1).axes.x.label || '', v, app.getGraphicsOptions(1).axes.z.label || ''),
        'zaxislabel': (app,v) => app.setAxisLabels(1, app.getGraphicsOptions(1).axes.x.label || '', app.getGraphicsOptions(1).axes.y.label || '', v),
        'axissteps': (app,v) => app.setAxisSteps(...splitlist(v).map(x=>parseFloat(x))),
    ...
    class GeogebraElement extends HTMLElement {
        ...
        setup_attribute_changers() {
            ...
            this.attribute_changers['pointcapture'] = {
                group: 'immediate',
                fn: (app,v) => app.setPointCapture(1, enum_attribute(v, ['none','snap','fixed','automatic']))
            };
            this.attribute_changers['sortalgebra'] = {
                group: 'immediate',
                fn: (app,v) => app.setAlgebraOptions({sortBy: enum_attribute(v, ['dependency','type','layer','order'])})
            };
    ...
    async set_graphics_options() {
        const app = await this.applet;
        const graphics_options = Object.fromEntries(graphics_option_names.filter(name=>this.hasAttribute(name)).map(name=>[name,this.cleanGetAttribute(name)]));
        graphics_options['rightAngleStyle'] = enum_attribute(this.getAttribute('rightanglestyle'), ['none','square','dot','l']);
    ...
    try_create(xml) {
        ...
        const options = Object.fromEntries(init_param_names.filter(name => this.hasAttribute(name)).map(name => [name, this.cleanGetAttribute(name)]));
        if(!xml) {
            if(this.hasAttribute('material')) {
                options.material_id = clean_material_id(this.cleanGetAttribute('material'));
            }
        }
        ...
        options.appletOnLoad = (app) => {
            ...
            app.registerClearListener(() => this.dispatchEvent(new CustomEvent('clear')));
            const other_events = [
                "Add",
                "Click",
                "Remove",
            ...
            other_events.forEach(event_name => {
                app[`register${event_name}Listener`]((name) => {
                    this.dispatchEvent(new CustomEvent(event_name.toLowerCase(), {detail: {object: name}}));
                })
            });
...
class GeoGebraObjectProxy {
    ...
    listValue(i) { return this.applet.getListValue(this.name, i); }
    get valueStringLocalized() { return this.applet.getValueString(this.name, true); }
    get commandLocalized() { return this.applet.getCommandString(this.name, true); }
    get LaTeXBase64Value() { return this.applet.getLaTeXBase64(this.name, true); }
    get labelStyleSubstituted() { return this.applet.getLabelStyle(this.name, true); }

    delete() {
        this.applet.deleteObject(this.name);
    }

    hide() { this.applet.setVisible(this.name, false); }
    show() { this.applet.setVisible(this.name, true); }
...
const object_proxy_properties = {
    'x': { get: 'getXcoord', set(x) { this.applet.setCoords(this.name, x, this.y, this.z) } },
    'y': { get: 'getYcoord', set(y) { this.applet.setCoords(this.name, this.x, y, this.z) } },
    'z': { get: 'getZcoord', set(z) { this.applet.setCoords(this.name, this.x, this.y, z) } },
    'value': { get: 'getValue', set: 'setValue' },
    'color': { get: 'getColor', set(color) { this.applet.setColor(this.name, ...parse_css_color(color)) } },
    'visible': { get: 'getVisible', set: 'setVisible' },
    'valueString': { get: 'getValueString' },
    'definition': { get: 'getDefinitionString' },
    'command': { get: 'getCommandString', set(cmd) { this.applet.evalCommand(`${this.name} = ${cmd}`) } },
    ...
    'lineStyle': { get: 'getLineStyle', set(v) { this.applet.setLineStyle(this.name, enum_attribute(v, ['full', 'dashed long', 'dashed short', 'dotted', 'dash-dot'])) } },
    'lineThickness': { get: 'getLineThickness', set: 'setLineThickness' },
    'pointStyle': { get: 'getPointStyle', set(v) { this.applet.setPointStyle(this.name, enum_attribute(v, ['filled circle', 'cross', 'circle', 'plus', 'filled diamond', 'unfilled diamond', 'triangle north', 'triangle south', 'triangle east', 'triangle west'])) } },
    'pointSize': { get: 'getPointSize', set: 'setPointSize' },
    'filling': { get: 'getFilling', set: 'setFilling' },
    'caption': { get: 'getCaption', set: 'setCaption' },
    'labelStyle': { get: 'getLabelStyle', set(v) { this.applet.setLabelStyle(this.name, enum_attribute(v, ['name', 'name value', 'value', 'caption'])) } },
...
Object.entries(object_proxy_properties).forEach(([prop,{get,set}]) => {
    Object.defineProperty(
        GeoGebraObjectProxy.prototype, 
        prop, 
        { 
            get() { return this.applet[get](this.name); } ,
            set(v) {
...
customElements.define('geogebra-applet', GeogebraElement);
